<?php	
           ...
           if(isset($_GET['follow_topic'])){
            ...
            $i = 0;
            $sql = mysqli_query($conn, 'SELECT `title`, `id`,`category_id` FROM `topics`');
            while ($row = mysqli_fetch_array($sql))
                {
                    $topics[$i]['title'] = $row['title'];
                    $topics[$i]['id'] = $row['id'];
                    $topics[$i]['category_id'] = $row['category_id'];
                    $i++;
                }
            
            $i = 0;
            $sql = mysqli_query($conn, "SELECT title, id FROM categories WHERE id>1");
            while ($row = mysqli_fetch_array($sql))
                {
...
<html>
    ...
    <body>
        ...
        <div id="main">
            ...
            <div class="topic-part"><a href="more2.php"><b>Suggested</b><hr color="DeepSkyBlue" width="75" size="3"></a></div>
            <div class="topic-part"><a href="more3.php">Not Interested</a><br></div>
            </div>

            <div class="moving">
            <br>
            <h2 class="title-topics">Categories</h2>
            <hr color="white">
                ...
                <div class="show">
                    <span class=blue>Show more</span>
                </div>
                ...
                <div class="topics-list">
                    ...
                    try { 
                        ...
                        ON topics.id = recommendedtopics.topic_id AND topics.category_id = $category_id";
                    ...
                    $categories_topics = array();
                    $i = 0;
                    
                    while ($row = mysqli_fetch_array($result))
                        {
                            $categories_topics[$i]['title'] = $row['title'];
                            $categories_topics[$i]['id'] = $row['id'];
                            $i++;
                        }
                    $topic_id = 1;?>

                    <?php foreach($categories_topics as $items): ?>
                        <?php if ($topic_id % 5 == 1){
                            echo '<div>';
                        }?>
                        <div class="topics-item">
                        <?php echo $items['title'];?>
                        <a href="more2.php?follow_topic=<?php echo $items['id']; ?>">
                        <img class="blue-img" src="https://img.icons8.com/android/344/plus.png" width="20">
                        </a>
                        </div>
                        <?php if (($topic_id %5 == 0)||$topic_id==count($categories_topics)){
                                echo '</div>';
                            } 
                        $topic_id++;?>
                    <?php endforeach; 
                ...
                <div class="view-all"><span class="blue">View All</span></div>
    ...
    <div class="follow">
        ...
        <?php foreach($whotofollow as $item): ?>
            ...
            $item_id = $item['user_id'];
                if ((bool)$isfollowed){
                    echo "<a href=\"more2.php?del_following=$item_id\">";
                    echo "<button id=\"unfollow-button\">Following</button></a>";
                }
                else {
                    echo "<a href=\"more2.php?del_follow=$item_id\">";
